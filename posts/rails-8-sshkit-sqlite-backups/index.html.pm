...
◊h1{Simple SQLite Backups for Rails 8 with SSHKit}
◊posting-date{2025-04-28}
...
◊section{
  ...
  Eventually, I realized that I could just use ◊code{docker volume inspect $VOLUME_NAME} and then parse out the mount point property.
  I tried to mess with the folder permissions so my admin user could access the docker volumes without using ◊code{sudo}, but I eventually gave up and started using ◊code{sudo}, and then everything worked perfectly.
  Sometimes, ◊a[#:href "https://en.wikipedia.org/wiki/Worse_is_better"]{worse really is better}.
}

◊section{
  This project was also a fun excuse to play around with ◊a[#:href "https://github.com/capistrano/sshkit"]{SSHKit}, which is a Ruby library for running commands on remote servers.
  My use case was really simple since I only have one host and I don't have any complex logic.
  My favorite feature is the ability to ◊a[#:href "https://github.com/capistrano/sshkit?tab=readme-ov-file#transferring-files"]{transfer files}.
