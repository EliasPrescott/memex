...
◊h1{Making a NeoVim Clone with Ruby}

◊posting-date{2025-05-06}

◊section{
  I've been learning Ruby recently.
  I've really been enjoying it and I've been reading through the Ruby docs quite a bit.
  When I found this section on ◊a[#:href "https://docs.ruby-lang.org/en/3.4/IO.html#class-IO-label-Extension+io-2Fconsole"]{io/console}, I thought it sounded like fun.
  ...
  A high-quality implementation of a text editor would require ◊i{tons} of code to handle all the little edge cases around text manipulation, buffer display, and cursor handling.
  ...
  NeoVim has a command mode (◊code{:help Cmdline-mode} in Vim to learn more) which allows users to run a variety of commands.
  ...
  To implement a basic version of this REPL, I made a ◊code{RVimCommandCenter} class which holds all the helper methods I want to expose to users.
  I made a basic eval method on ◊code{RVimCommandCenter} which will evaluate input within the context of the class instance by using ◊code{binding.eval input}.
  Any code evaluated using this method will have immediate access to the methods within ◊code{RVimCommandCenter}.

  So, to implement the ◊code{q} command to quit the editor, I can simply do this:
...
◊section{
  ...
  To provide a better REPL experience while I am still building out the command mode, I also added a ◊code{dbg} method to the command center which will place the console back into cooked mode and open ◊a[#:href "https://github.com/ruby/irb"]{irb}.
  ...
  ◊yt["RVim Demo #1"]{https://www.youtube-nocookie.com/embed/hzL5Su0PDo0?si=w15WHxyCsWxge_fd}
  ...
  ◊a[#:href "https://github.com/EliasPrescott/rvim"]{Here} is the repo for RVim if you want to look at the code or play around with it.
  You should be able to run ◊code{bundle} or ◊code{gem install -g}, and then just ◊code{ruby main.rb}.
}
